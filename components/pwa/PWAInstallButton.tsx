  ...
  if (isInstallable) {
    return (
      <button
        ...
        onClick={install}
        className="inline-flex items-center gap-2 border border-[#2A2927] hover:border-[#C5A880] px-3.5 py-1.5 rounded-full text-xs tracking-wider text-[#AAA69D] hover:text-[#F2F0EA] transition-colors duration-300 uppercase"
    ...
    return (
      <>
        ...
        {showIOSGuide && (
          ...
          >
            <div
              className="w-full max-w-sm border border-[#2A2927] bg-[#111111] p-6 shadow-2xl relative text-[#F2F0EA]"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                onClick={() => setShowIOSGuide(false)}
                className="absolute top-4 right-4 text-[#AAA69D] hover:text-[#F2F0EA]"
                aria-label="Zamknij"
              >
                <X className="w-5 h-5" />
              </button>
              <h3 className="font-serif-luxury text-xl text-[#F2F0EA] mb-3">
              ...
              <button
                onClick={() => setShowIOSGuide(false)}
                className="mt-6 w-full border border-[#2A2927] hover:border-[#C5A880] py-2 text-xs uppercase tracking-widest text-[#F2F0EA] transition-colors"
              >
                Rozumiem
              </button>
            </div>
